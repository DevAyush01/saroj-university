                ...
                <AnimatePresence>
                  {expandedSections[program.id] && (
                    ...
                    >
                      ...
                      {/* Modes and Fees Section */}
                      {program.modes && program.modes.length > 0 && (
                        <div className="mt-6">
                          <h4 className="font-semibold text-gray-800 mb-3 flex items-center">
                            <FaChartBar className="mr-2 text-blue-600" /> Program Modes & Fees:
                          </h4>
                          <div className="space-y-4">
                            {program.modes.map((modeInfo, index) => (
                              <div key={index} className="bg-white p-4 rounded-lg shadow-sm border border-gray-100">
                                <div className="flex items-center mb-2">
                                  {modeInfo.modeName.includes('Global') ? (
                                    <FaGlobe className="mr-2 text-blue-600" />
                                  ) : (
                                    <FaHome className="mr-2 text-green-600" />
                                  )}
                                  <h5 className="font-bold text-lg text-gray-800">{modeInfo.modeName}</h5>
                                </div>
                                <p className="text-gray-600 text-sm mb-3 ml-6">{modeInfo.modeDescription}</p>
                                <div className="ml-6 grid grid-cols-2 gap-y-2 text-sm text-gray-700">
                                  <div>Year 1 Fees: <span className="font-medium">₹{modeInfo.fees.year1.toLocaleString('en-IN')}</span></div>
                                  <div>Year 2 Fees: <span className="font-medium">₹{modeInfo.fees.year2.toLocaleString('en-IN')}</span></div>
                                  <div>Year 3 Fees: <span className="font-medium">₹{modeInfo.fees.year3.toLocaleString('en-IN')}</span></div>
                                  <div className="col-span-2 mt-2 pt-2 border-t border-gray-200">
                                    <p className="font-bold text-base text-gray-800">Total Fees: ₹{modeInfo.fees.total.toLocaleString('en-IN')}</p>
                                  </div>
                                </div>
                              </div>
                            ))}
