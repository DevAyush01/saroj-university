...
const CoursePage = ({ universityData }) => {
  ...
  const allDegreeTypes = [...new Set(universityData.institutes.flatMap(i => i.programs.map(p => p.degree)))];
  ...
  const filteredPrograms = allPrograms.filter(program => {
    const matchesSearch = 
      (typeof program.degree === 'string' && program.degree.toLowerCase().includes(searchTerm.toLowerCase())) ||
      (typeof program.mode === 'string' && program.mode.toLowerCase().includes(searchTerm.toLowerCase())) ||
      (
        program.specializations &&
        (
          (typeof program.specializations === 'string' && program.specializations.toLowerCase().includes(searchTerm.toLowerCase())) ||
          (Array.isArray(program.specializations) && JSON.stringify(program.specializations).toLowerCase().includes(searchTerm.toLowerCase()))
        )
      ) ||
      (typeof program.instituteName === 'string' && program.instituteName.toLowerCase().includes(searchTerm.toLowerCase()));
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
          <div className="text-sm text-gray-600">
            Showing <span className="font-semibold">{tabGroups[activeCategoryTab].length}</span> of <span className="font-semibold">{allPrograms.length}</span> programs
          </div>
